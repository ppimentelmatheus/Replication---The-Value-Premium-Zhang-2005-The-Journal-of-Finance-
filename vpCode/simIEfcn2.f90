...
elseif(nlhs .ne. 2) then
	call mexErrMsgTxt('2 output variables required.')
endif
...
call simIEfcnP( %val(mxGetPr(plhs(1))), %val(mxGetPr(plhs(2))),                                       &
...
! [simh, sigk] = simIEfcnP(kd0, zd0, sx, optK, k, nk, x, nx, z, h, nh, N, Ts, alpha, eta, rhoz, stdz)

subroutine simIEfcnP(simh, sigk, kd0, zd0, sx, optK, k, nk, x, nx, z, h, nh, N, Ts, alpha, eta, rhoz, stdz, nz)
...
real*8, intent(out) :: simh(Ts), sigk(Ts)

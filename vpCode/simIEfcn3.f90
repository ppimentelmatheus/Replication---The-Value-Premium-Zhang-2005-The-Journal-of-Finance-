...
!--------------------------------------------------------------
!                 MEX file for SIMIEFCNP routine
!--------------------------------------------------------------
implicit none
...
plhs(1)   = mxCreateFull(1, Ts, 0)
plhs(2)   = mxCreateFull(1, Ts, 0)
plhs(3)   = mxCreateFull(1, Ts, 0)

call simIEfcnP( %val(mxGetPr(plhs(1))), %val(mxGetPr(plhs(2))), %val(mxGetPr(plhs(3))),                             &
...
! [simh, sigk, sigz] = simIEfcnP(kd0, zd0, sx, optK, k, nk, x, nx, z, h, nh, N, Ts, alpha, eta, rhoz, stdz)

subroutine simIEfcnP(simh, sigk, sigz, kd0, zd0, sx, optK, k, nk, x, nx, z, h, nh, N, Ts, alpha, eta, rhoz, stdz, nz)
...
real*8, intent(out) :: simh(Ts), sigk(Ts), sigz(Ts)
...
real*8  :: kd(N), zd(N), kpd(N), optK_x(nk, nh, nz), optK_xh(nk, nz), optK_xhk(nz), Y(Ts)
...
do t = 1, Ts
    ! aggregate output
    Y(t)    = sum(dexp(sx(t) + zd)*(kd**alpha))
    ! log output price --- N is a scaling factor
    simh(t) = -eta*dlog(Y(t)/N)
	! cross-sectional volatility of capital
	sigk(t) = sqrt(sum((kd - sum(kd)/N)**2)/(N - 1))
	sigz(t) = sqrt(sum((zd - sum(zd)/N)**2)/(N - 1))
    ...
    ! update next period capital stock
    kd = kpd
    ...
    zd = max(-3.5*(stdz/sqrt(1 - rhoz**2)), min(3.5*(stdz/sqrt(1 - rhoz**2)), rhoz*zd + stdz*shocFull))

    ! visual
	if (modulo(t, 2400) == 0) then         
		lapse = secnds(T1)      	
		statConsole = AllocConsole()
		print "(a, i8, a, f8.1, a)", " periods simulated:", t, "   Time:", lapse, "s"
	end if
end do
